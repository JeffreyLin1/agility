...
function resolvePlaceholders(text, context) {
  ...
  return text.replace(/{{input\.(.*?)}}/g, (match, path)=>{
    ...
    const stringValue = typeof value === 'string' ? value : value === null || value === undefined ? '' : JSON.stringify(value);
...
function resolvePlaceholdersInObject(obj, context) {
...
serve(async (req)=>{
  ...
  try {
    ...
    if (!startElementId) {
      return new Response(JSON.stringify({
        error: 'Start element ID is required'
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    if (parts.length > 0 && uuidRegex.test(parts[0])) {
      workflowId = parts[0];
      debug('Found UUID in element ID', {
        workflowId
      });
    }
    ...
    if (!workflowId) {
      ...
      const { data: workflows, error: workflowsError } = await supabaseClient.from('user_workflows').select('id, data').eq('user_id', user.id);
      if (workflowsError) {
      ...
      for (const workflow of workflows || []){
        ...
        if (elements.some((element)=>element.id === startElementId)) {
          ...
          debug('Found workflow containing element', {
            workflowId
          });
          break;
        }
    ...
    const { data: workflow, error: workflowError } = await supabaseClient.from('user_workflows').select('*').eq('id', workflowId).eq('user_id', user.id).single();
    if (workflowError) {
    ...
    // Create a map of connections for easy lookup - modified to support multiple targets
    ...
    debug('Connection map created', {
      connectionCount: connections.length,
      uniqueSourcesCount: connectionMap.size,
      sources: Array.from(connectionMap.keys())
    });
    ...
    const outputContext = {};
    // Use a queue to process elements in breadth-first order
    const queue = [
      startElementId
    ];
    ...
    while(queue.length > 0){
      ...
      if (processed.has(currentElementId)) {
        debug('Skipping already processed element', {
      ...
      if (currentElement.type === 'agent') {
        ...
        const { data: config, error: configError } = await supabaseClient.from('agent_configs').select('config').eq('element_id', currentElementId).eq('user_id', user.id).eq('agent_type', agentType).maybeSingle();
        if (configError) {
        ...
        if (agentType === 'text_generator') {
          ...
          if (result && typeof result === 'object' && 'text' in result) {
            ...
            debug('No text in result, storing error message', {
              error: result.error
            });
          }
          ...
          try {
            ...
            if (result.messages && result.messages.length > 0) {
              ...
              debug('No messages found in Gmail reader result', {
                result: JSON.stringify(result).substring(0, 100)
              });
            ...
            debug('Error parsing Gmail reader response JSON', {
              error: e.message,
              responseText
            });
            result = {
          ...
          try {
            ...
            const responseText = await response.text();
            debug('Discord messenger raw response', {
              responseText
            });
            try {
              result = JSON.parse(responseText);
      ...
      debug('Next elements in workflow', {
        currentElementId,
        nextElementIds,
        count: nextElementIds.length
      });
